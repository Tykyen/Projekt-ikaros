...
| P0 | PT-35e | Stale role: demotovaný Admin (JWT `role:Admin`) čte cizí private svět; `OptionalJwtAuthGuard` NEobnovuje roli z DB → bypass do expirace (3 dny) | `GET /worlds/:id`; `optional-jwt-auth.guard.ts` | Optional guard bere roli z DB + degraduje banned/deleted | 35 | T1 | 🟢 **ZAVŘENO** (fix+4 testy `1fc63d1`) |
...
| — | PT-39a | ~~Ne-podporovatel uloží prémiový `chatSkin` (gate jen na `diceSkinMapping`)~~ **NENÍ DÍRA — útok stál na neexistující featuře.** Žádný free/prémiový split světových motivů neexistuje (`theme-ids.ts:52-65` = 12 motivů bez markeru; free tier je jen u kostek, prefix `bezne-*`). Produktově potvrzeno 2026-07-17: **prémiové motivy nebudou** → gate nemá co vynucovat. Kdyby se to otočilo, gate musí padnout i na `WorldMembership.themeId` a `diarySkin`, jinak je obejitelný. (Řádky v původním zápisu byly navíc posunuté: realita `chat.service.ts:2544`, gate `:2604`.) | `PATCH .../chat/appearance` | — | 39 | — | ⚪ **ZAVŘENO (není zranitelnost)** |
