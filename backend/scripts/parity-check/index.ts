...
async function main(): Promise<void> {
  ...
  console.log(
    `   Endpointy: ${oldData.endpoints.length}, Hub metody: ${oldData.hubMethods.length}, Schémata: ${oldData.schemas.length}, Joby: ${oldData.cronJobs.length}, JWT claims: ${oldData.jwtClaims.length}`,
  );

  console.log('🔍 Parsuju nový NestJS backend...');
  const newData = parseNestjsBackend(NEW_BACKEND_SRC);
  console.log(
    `   Endpointy: ${newData.endpoints.length}, Gateway zprávy: ${newData.gatewayMessages.length}, Schémata: ${newData.schemas.length}, Cron joby: ${newData.cronJobs.length}, JWT claims: ${newData.jwtClaims.length}`,
  );
  ...
  const missingCount =
    diff.endpoints.missing.length +
    diff.hubMethods.missing.length +
    diff.schemas.missing.length +
    diff.cronJobs.missing.length +
    diff.jwtClaims.missing.length;
