...
function parseControllerFile(content: string): OldEndpoint[] {
  ...
  const httpPattern =
    /\[(HttpGet|HttpPost|HttpPut|HttpDelete|HttpPatch)(?:\("([^"]*)"\))?\]/g;
  let match;
...
function parseModelFiles(backendDir: string): OldSchema[] {
  ...
  if (fs.existsSync(settingsPath)) {
    ...
    const propPattern =
      /public string (\w+CollectionName)\s*\{[^}]+\}\s*=\s*"([^"]+)"/g;
    let match;
...
function parseJwtClaims(content: string): OldJwtClaim[] {
  ...
  while ((match = pattern.exec(content)) !== null) {
    ...
    claimType = claimType
      ...
      .replace(/JwtRegisteredClaimNames\.(\w+)/, (_, n: string) =>
        n.toLowerCase(),
      );
    seen.add(claimType);
  }
...
export function parseCsharpBackend(backendDir: string): OldBackendData {
  ...
  for (const file of (fs.existsSync(controllersDir)
  ...
  for (const file of (fs.existsSync(hubsDir)
    ? fs.readdirSync(hubsDir)
    : []
  ).filter((f) => f.endsWith('.cs'))) {
