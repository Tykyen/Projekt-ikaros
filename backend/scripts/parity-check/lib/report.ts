...
export function generateMarkdown(input: ReportInput): string {
  ...
  lines.push(
    `| REST endpointy | ${epOld} | ${ep.covered.length} | ${ep.renamed.length} | ${ep.missing.length} | ${ep.extra.length} | ${statusIcon(ep.missing.length > 0)} |`,
  );

  const hub = diff.hubMethods;
  const hubOld = input.oldHubMethodCount;
  lines.push(
    `| WebSocket události | ${hubOld} | ${hub.covered.length} | — | ${hub.missing.length} | ${hub.extra.length} | ${statusIcon(hub.missing.length > 0)} |`,
  );

  const sc = diff.schemas;
  const scOld = input.oldSchemaCount;
  lines.push(
    `| MongoDB schémata | ${scOld} | ${sc.covered.length} | — | ${sc.missing.length} | ${sc.extra.length} | ${statusIcon(sc.missing.length > 0)} |`,
  );

  const cr = diff.cronJobs;
  const crOld = input.oldCronJobCount;
  lines.push(
    `| Cron joby | ${crOld} | ${cr.covered.length} | — | ${cr.missing.length} | ${cr.extra.length} | ${statusIcon(cr.missing.length > 0)} |`,
  );

  const jwt = diff.jwtClaims;
  const jwtOld = input.oldJwtClaimCount;
  lines.push(
    `| JWT claims | ${jwtOld} | ${jwt.covered.length} | — | ${jwt.missing.length} | ${jwt.extra.length} | ${statusIcon(jwt.missing.length > 0)} |`,
  );
