...
const ADMIN_ROLES = [UserRole.Superadmin, UserRole.Admin, UserRole.PJ, UserRole.SpravceGalerie];
...
export class IkarosGalleryService {
  ...
  async create(
    ...
    if (status === 'Pending') {
      await this.notifyAdmins('Obrázek čeká na schválení', `/ikaros/galerie/${item.id}`);
    }
    return item;
  }
  ...
  async submit(id: string, userId: string, role: UserRole): Promise<IkarosGalleryItem> {
    ...
    await this.notifyAdmins('Obrázek čeká na schválení', `/ikaros/galerie/${id}`);
    return updated!;
  }
