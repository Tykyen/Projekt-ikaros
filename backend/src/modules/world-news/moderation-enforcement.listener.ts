...
/**
 * Fáze B5 (spec 20B) — vynucení content-level moderačních zásahů nad NOVINKAMI
 * světa. Naslouchá generickému `moderation.enforce` / `moderation.revert`
 * a reaguje jen na `targetType === world_news`:
 *   - M2/M3 (skrytí)   → `moderationHidden = true`  (revert → false)
 *   - M4 (odstranění)  → hard delete (NEVRATNÝ; revert jen log)
 *   - M5–M7            → ignoruje (account-level řeší modul `users`)
 *
 * Best-effort — na neznámém id / chybě jen zaloguje, nikdy neshodí `resolveReport`.
 */
@Injectable()
export class WorldNewsModerationEnforcementListener {
  private readonly logger = new Logger(
    WorldNewsModerationEnforcementListener.name,
  );
  ...
  async onEnforce(p: ModerationEnforcePayload): Promise<void> {
    if (p.targetType !== ReportTargetType.WorldNews) return;
    try {
      switch (p.action) {
        ...
        case ModerationAction.Remove: {
          const ok = await this.service.moderationDelete(p.targetId);
          this.logger.log(
            ok
              ? `Novinka ${p.targetId} smazána moderací (M4) — rozhodnutí ${p.decisionId}.`
              : `Smazání novinky ${p.targetId} (M4) — nenalezena (rozhodnutí ${p.decisionId}).`,
          );
          break;
        }
        default:
      ...
      logError(
        this.logger,
        `Enforcement novinky ${p.targetId} (${p.action}, rozhodnutí ${p.decisionId}) selhal`,
        err,
      );
    }
  }
  ...
  async onRevert(p: ModerationEnforcePayload): Promise<void> {
    ...
    try {
      switch (p.action) {
        ...
        case ModerationAction.Remove:
          this.logger.warn(
            `Revert M4 — smazanou novinku ${p.targetId} nelze vrátit (rozhodnutí ${p.decisionId}).`,
          );
          break;
        default:
          break;
      ...
      logError(
        this.logger,
        `Revert novinky ${p.targetId} (${p.action}, rozhodnutí ${p.decisionId}) selhal`,
        err,
      );
    }
  }
  ...
  private async setHidden(
    ...
    if (!ok) {
      this.logger.warn(
        `${hidden ? 'Skrytí' : 'Odkrytí'} novinky ${p.targetId} — nenalezena (rozhodnutí ${p.decisionId}).`,
      );
      return;
    }
    this.logger.log(
      `Novinka ${p.targetId} ${hidden ? 'skryta' : 'odkryta'} moderací — rozhodnutí ${p.decisionId}.`,
    );
  }
}
