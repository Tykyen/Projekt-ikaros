...
describe('NpcTemplatesService', () => {
  ...
  describe('findOne', () => {
    ...
    it('vyhodí NotFoundException pokud šablona neexistuje', async () => {
      ...
      await expect(service.findOne('tpl1', 'world1')).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('update', () => {
    it('vrátí aktualizovanou šablonu', async () => {
      ...
      const result = await service.update('tpl1', 'world1', {
        name: 'Super Goblin',
      });
      expect(result.name).toBe('Super Goblin');
      expect(mockRepo.updateByIdAndWorld).toHaveBeenCalledWith(
        'tpl1',
        'world1',
        expect.objectContaining({ name: 'Super Goblin' }),
      );
    });
    ...
    it('vyhodí NotFoundException pokud repo vrátí null (šablona neexistuje nebo jiný world)', async () => {
      ...
      await expect(
        service.update('tpl1', 'world1', { name: 'X' }),
  ...
  describe('remove', () => {
    it('úspěšně smaže šablonu', async () => {
      ...
      expect(mockRepo.deleteByIdAndWorld).toHaveBeenCalledWith(
        'tpl1',
        'world1',
      );
    });
    ...
    it('vyhodí NotFoundException pokud repo vrátí false', async () => {
      ...
      await expect(service.remove('tpl1', 'world1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('importToWorld', () => {
    ...
    it('vyhodí NotFoundException pokud globální šablona neexistuje', async () => {
      mockRepo.findById.mockResolvedValue(null);
      await expect(service.importToWorld('bad', 'world1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
