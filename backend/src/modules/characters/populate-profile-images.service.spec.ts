...
describe('PopulateProfileImagesService', () => {
  ...
  describe('populateFromCharacter', () => {
    it('nastaví profileImageUrl pokud user ho nemá', async () => {
      ...
      await service.populateFromCharacter({
        userId: 'u1',
        imageUrl: 'https://img.example.com/a.jpg',
        isNpc: false,
      });
      expect(mockUsersRepo.update).toHaveBeenCalledWith('u1', {
        profileImageUrl: 'https://img.example.com/a.jpg',
      });
    });
    ...
    it('nepřepíše profileImageUrl pokud user ho má', async () => {
      mockUsersRepo.findById.mockResolvedValue({
        id: 'u1',
        profileImageUrl: 'https://img.example.com/existing.jpg',
      });
      await service.populateFromCharacter({
        userId: 'u1',
    ...
    it('přeskočí NPC (bez userId)', async () => {
      await service.populateFromCharacter({
        userId: undefined,
        imageUrl: 'https://img.example.com/npc.jpg',
        isNpc: true,
      });
