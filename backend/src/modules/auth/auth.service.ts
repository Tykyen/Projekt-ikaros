...
// 23.7 — hodnota přes env `REFRESH_REUSE_GRACE_MS` (default níž), aby šla
// security regrese testovat v CI (e2e nastaví 0 → reuse hned spustí detekci).
const DEFAULT_REFRESH_REUSE_GRACE_MS = 60 * 1000;

@Injectable()
export class AuthService {
  private readonly logger = new Logger(AuthService.name);

  // Anti-enumeration (timing) — login s neexistujícím identifierem musí trvat
  // ~stejně jako s existujícím (bcrypt.compare ~100 ms). Reálný hash (cost 10
  // jako registrace) spočtený při startu; heslo je irelevantní, compare vždy
  ...
  // 23.7 — grace okno v ms (default 60 s). Env override hlavně pro e2e (0 =
  // okno vypnuté → reuse hned padá do detekce). Nevalidní/záporné → default.
  private get refreshGraceMs(): number {
    const raw = this.config.get<string>('REFRESH_REUSE_GRACE_MS');
    const n = raw !== undefined ? Number(raw) : NaN;
    return Number.isFinite(n) && n >= 0 ? n : DEFAULT_REFRESH_REUSE_GRACE_MS;
  }
  ...
  async login(dto: LoginDto, trustToken?: string): Promise<LoginResult> {
    ...
    if (!user) {
      // Anti-enumeration — bez dummy compare by neexistující účet odpověděl
      // o ~100 ms rychleji než existující (chybí bcrypt práce) → měřitelná
      // enumerace identifierů. Výsledek zahazujeme, vždy INVALID_CREDENTIALS.
      await bcrypt
        .compare(dto.password, await this.timingDummyHash)
        .catch(() => false);
  ...
  async reactivateDeletion(dto: LoginDto): Promise<LoginResult> {
    ...
    const user = isEmail
      ...
      : await this.usersRepo.findByUsername(dto.identifier);
    if (!user) {
      // Anti-enumeration — stejný timing-equalizer jako `login`.
      await bcrypt
        .compare(dto.password, await this.timingDummyHash)
        .catch(() => false);
      throw new UnauthorizedException({
  ...
  private rememberGracePair(
    ...
    this.refreshGrace.set(oldJti, {
      pair,
      expiresAt: now + this.refreshGraceMs,
    });
  }
