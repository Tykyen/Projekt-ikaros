...
class WeatherGeneratorConfigSchema {
  ...
  @Prop({
    ...
    default: (): WeatherTypeEntrySchema[] => [],
  })
  weatherTypes: WeatherTypeEntrySchema[];
  ...
  @Prop({
    type: [MixedArraySubSchema],
    default: (): CustomFieldConfigSchema[] => [],
  })
...
export class WeatherGeneratorSchemaClass {
  ...
  // 9.4-I — sdílené pořadí karet generátorů per svět. Default 0 = end of list (backfill po prvním reorderu).
...
// 9.4-I — compound index pro řazení v listu generátorů
WeatherGeneratorSchema.index({ worldId: 1, displayOrder: 1, createdAt: 1 });
