...
export class PendingActionsService {
   ...
   * `canHandle(role) === true`. Použito pro badge v pravém panelu / tab nav.
   *
   * Spec 3.8 — vedle `total` vrací i `byType` rozpad (počet per queue typ).
   * `byType` obsahuje jen typy, které uživatel přes `canHandle` vidí — FE
   * z toho odvozuje per-doména badge u nav položek (Diskuze/Články/Galerie)
   * bez vlastní role logiky.
   */
  async countForUser(
    userId: string,
    role: UserRole,
