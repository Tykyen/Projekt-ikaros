...
export class MapsGateway implements OnGatewayConnection {
  ...
  // reload-scene/scene-cleared/effect-*/fog-updated/dice-rolled/
  // scene-state-changed/sound-changed) ODSTRANĚNY. Nahrazeny operation modelem
  // (10.2-prep-1+); FE je už neemitoval ani neposlouchal (ověřeno grepem) →
  // byly to mrtvý kód + latentní relay surface (klient mohl broadcastnout
  // falešný stav scény mimo operation log). Živé zůstávají: map:ping, spotlight.
