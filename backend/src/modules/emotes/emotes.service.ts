...
export class EmotesService {
  ...
  async create(
    ...
    if (existing)
      throw new ConflictException(`Shortcode :${dto.shortcode}: je již použit`);
  ...
  async createGlobal(
    ...
    if (existing)
      throw new ConflictException(
        `Shortcode :${dto.shortcode}: je již použit globálně`,
      );
  ...
  async deleteFromWorld(id: string, worldId: string): Promise<void> {
    ...
    if (!emote || emote.worldId !== worldId)
      throw new NotFoundException('Emote nenalezen');
  ...
  async deleteGlobal(id: string): Promise<void> {
    ...
    if (!emote || emote.worldId !== null)
      throw new NotFoundException('Globální emote nenalezen');
    await this.repo.deleteById(id);
  }
  ...
  async copy(
    ...
    if (collision)
      throw new ConflictException(
        `Shortcode :${emote.shortcode}: již existuje v cílovém světě`,
      );
