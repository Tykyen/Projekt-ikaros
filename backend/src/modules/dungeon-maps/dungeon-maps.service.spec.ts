...
describe('DungeonMapsService', () => {
  ...
  describe('assertCanManage', () => {
    ...
    it('hodí ForbiddenException pro hráče bez PJ role', async () => {
      ...
      await expect(service.assertCanManage('u1', UserRole.Hrac, 'world1')).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('create', () => {
    it('vytvoří dungeon s worldId z DTO', async () => {
      ...
      const result = await service.create({ worldId: 'world1', name: 'Kobka' }, 'pj1', UserRole.Hrac);
      expect(result).toEqual(mockDungeon);
  ...
  describe('replace', () => {
    it('nahradí dungeon', async () => {
      ...
      const result = await service.replace('dun1', { name: 'Nové jméno' }, 'pj1', UserRole.Hrac);
      expect(result).toEqual(updated);
    });

    it('hodí NotFoundException pokud dungeon neexistuje', async () => {
      mockRepo.findById.mockResolvedValue(null);
      await expect(service.replace('x', {}, 'pj1', UserRole.Hrac)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('delete', () => {
    it('smaže dungeon', async () => {
      ...
      await expect(service.delete('dun1', 'pj1', UserRole.Hrac)).resolves.toBeUndefined();
    });

    it('hodí NotFoundException pokud dungeon neexistuje', async () => {
      mockRepo.findById.mockResolvedValue(null);
      await expect(service.delete('x', 'pj1', UserRole.Hrac)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('exportTemplate', () => {
    it('vytvoří MapTemplate z dungeonu a vrátí templateId', async () => {
      ...
      const result = await service.exportTemplate('dun1', 'https://example.com/img.png', 'pj1', UserRole.Hrac);
    ...
    it('hodí NotFoundException pokud dungeon neexistuje', async () => {
      mockRepo.findById.mockResolvedValue(null);
      await expect(service.exportTemplate('x', 'https://img.png', 'pj1', UserRole.Hrac)).rejects.toThrow(NotFoundException);
    });
  });
